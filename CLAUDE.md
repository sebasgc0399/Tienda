...
- El cliente de Supabase vive en `lib/` con cuatro variantes (detalle de seguridad de cada una en la sección 9):
  ...
  2. **Cliente de servidor por sesión** — `createServerClient` (`@supabase/ssr`), `anon key` + cookies vía `next/headers`, sujeto a RLS. Sirve para lecturas que dependen de la sesión (panel admin, futuras lecturas autenticadas), no para el catálogo público.
  3. **Cliente público de servidor** — `createClient` (`@supabase-js`) sin cookies (`src/lib/supabase/public.ts`), `anon key`, sujeto a RLS. Es el cliente que usan los Server Components del catálogo público (home, listado, detalle): al no llamar `cookies()`, esas páginas quedan compatibles con ISR (`revalidate`) en vez de forzarse a renderizado dinámico.
  4. **Cliente admin/service** — `createClient` (`@supabase-js`), `service_role key`, evita RLS por completo, server-only. Uso concreto en v1: las mutaciones del admin (Server Actions en `features/admin/actions/` — filas de `categories`/`products`/`product_images` y objetos de Storage), siempre después de que la propia Server Action re-verificó la sesión con `getUser()`/`getClaims()` (ver sección 9) — esa re-verificación, no RLS, es lo que impide que cualquiera dispare estas mutaciones. Nunca se usa para renderizar páginas públicas ni anónimas.
...
### Imágenes de producto: siempre `next/image`

Las imágenes de producto y de categoría se renderizan siempre con el componente `Image` de `next/image`, nunca con un `<img>` nativo — es lo que da optimización automática y evita layout shift (CLS). Como las URLs salen del bucket público de Supabase Storage (dominio externo, no `public/`), hace falta declarar el hostname del proyecto en `images.remotePatterns` de `next.config.js`; sin esa entrada `next/image` rechaza la imagen.

El tier Hobby de Vercel tiene cuota de transformaciones de imagen (ver la nota de riesgo aceptado en [ADR-0004](docs/adr/0004-vercel-deployment.md)). Si esa cuota llega a ser un problema real, la salida documentada es `unoptimized={true}` sobre las imágenes que vienen del loader remoto de Supabase — sirve la imagen tal cual, sin pasar por el optimizador de Vercel. No es el comportamiento default; es el fallback documentado para ese escenario puntual, no una razón para evitar `next/image` desde el principio.
...
|---|---|---|
| Nunca commitear secretos ni `.env` | `.env` fuera de git desde el primer commit; credenciales de Supabase solo por variable de entorno. Detalle en la spec. | [admin-panel.md](docs/specs/admin-panel.md#seguridad) |
| `service_role key` solo en servidor | Nunca en un archivo `"use client"` ni con prefijo `NEXT_PUBLIC_`, nunca para renderizar páginas públicas. Detalle en la spec. | [admin-panel.md](docs/specs/admin-panel.md#seguridad) |
| Respetar RLS en toda consulta nueva | Ninguna tabla otorga policies de escritura a `anon`/`authenticated`; esquema completo de policies en la spec. | [data-model.md](docs/specs/data-model.md#row-level-security-rls), [admin-panel.md](docs/specs/admin-panel.md#seguridad) |
