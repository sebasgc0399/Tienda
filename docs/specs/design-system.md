...
| Listas | Tabla en desktop, tarjetas en mobile; miniatura, toggles inline y controles ▲▼ para `display_order` — mismo patrón en productos y categorías. Filas de producto agrupadas por categoría con un subencabezado discreto por grupo (texto muted, `text-xs uppercase tracking-wide`, fondo sutil); el nombre (de producto o categoría) es un link a su edición, mismo estilo de texto con subrayado al hover. Alta/baja de un producto se gestiona solo con el Switch `Activo` — sin botón destructivo por fila. En categorías, "Eliminar" por fila no es redundante con el Switch (corre un flujo de reasignar-y-eliminar que el Switch no puede expresar), así que se mantiene, pero demotado a ícono ghost (`Trash2`, rojo solo en el ícono) que abre el Dialog de confirmación — el destructivo relleno queda reservado al botón de confirmación dentro de ese Dialog. En las cards mobile, cada Switch lleva una etiqueta visible junto al control (ej. "Destacado", "Activo"), no solo `aria-label`. Controles ▲▼ como íconos ghost compactos, mismo tratamiento que las acciones de imagen. |
| Edición de producto/categoría | Layout de dos columnas en desktop (`datos` 1fr / zona secundaria 380px — `imágenes` en producto, `portada` en categoría), apiladas en mobile; encabezado de página con enlace de vuelta, nombre y badge de estado (Activo/Inactivo, Activa/Inactiva). Cada zona es un `Card` con heading real (`CardTitle`, `text-lg`) en vez de un `<h2>` suelto — una sola acción primaria por página (Guardar). |
